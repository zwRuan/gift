...
def calculate_pass_at_k(score_mat, k):
    """计算 pass@k: 前k个预测中至少有一个正确的样本比例"""
    pass_at_k_scores = []
    for scores in score_mat:
        # 取前k个预测，如果有任何一个正确就算通过
        pass_at_k = any(scores[:min(k, len(scores))])
...
def evaluate(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False):
    ...
    # 计算所有 pass@k 指标，从 1 到最大采样次数
    pass_at_k_results = {}
    for k in range(1, max_len + 1):
        pass_at_k_results[f"pass@{k}"] = np.round(calculate_pass_at_k(score_mat, k) * 100, 1)
    ...
    result_json = {
        ...
        "max_sampling": max_len,  # 添加最大采样次数信息
        "acc": mean_score[0],  # 保持原有的acc字段兼容性
        **pass_at_k_results  # 添加所有 pass@k 结果
    }
